...
const Rsa = (props) => {
    ...
    let checkerQ = (q) => {
        ...
        if (typeof q !== "number"){
            if (!props.isPrime(+q)){
                return <Error numb={'q'}></Error>
            }
        }
    }
    let encM = () => {
        setEnc(props.encMess(encrMessage, alphabet, p, q))
    }
    let decM = () => {
        setDec(props.decMess(dencrMessage, alphabet, p, q))
    }
    let handleChange = e => {
    ...
    return <div
        ...
        <div className="border-b-2 border-purple-400 mb-2 pb-1">
            <h2 className="text-2xl">{props.asymNames[0].name}</h2>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8 sm:grid-cols-1">
            <div className="">
                <div className="flex flex-col">
                    ...
                    {checkerP(p)}
                    <div className="flex">
                        <p className="mr-4">Просте число q: </p>
                        <input className="mb-2 w-24 bg-violet-50 outline-0 border-b-2 border-purple-400" type="text" name="q" value={q} onChange ={e => setQ(e.target.value.toUpperCase())} placeholder='19'></input>
                    </div>
                    {checkerQ(q)}
                </div>
            ...
            <div>
                <div className="flex flex-col">
                    ...
                    <div className="flex">
                        <p className="mr-4">Просте число q: </p>
                        <input className="mb-2 w-48 bg-violet-50 outline-0 border-b-2 border-purple-400" type="text" name="q" value={q} onChange ={e => setQ(e.target.value.toUpperCase())} placeholder='19'></input>
                    </div>
                    {checkerQ(q)}
                </div>
                <div className="flex flex-col mt-4">
